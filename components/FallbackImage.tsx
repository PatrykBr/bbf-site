...
function FallbackImageComponent({
    ...
    if (error) {
        const fallbackClassName = `from-brand-light/20 to-brand-dark/20 flex items-center justify-center bg-linear-to-br ${fill ? "absolute inset-0" : ""} ${className}`;
        ...
        if (onClick) {
            return (
                <button
                    type="button"
                    className={`${fallbackClassName} cursor-pointer appearance-none border-0 p-0`}
        ...
        return (
            <div className={fallbackClassName} style={!fill ? { width, height } : undefined}>
                {fallbackContent}
